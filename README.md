...
It provides the backend functionality for the Coderr platform, including
authentication, customer and business profiles, offers, orders, reviews,
platform statistics, media uploads, and role-based permissions.

The project uses token-based authentication and is designed to work with the
separate Coderr frontend.
...
This step is optional, but required if you want to use the Django admin
interface.
...
Coderr uses two application-specific profile types in addition to Django staff
permissions.
...
Each app keeps API-related code inside its own `api/` directory,
including serializers, views, permissions, and URL configuration.
## Environment Variables

The project reads sensitive and environment-specific settings from a `.env`
file.
...
After a successful registration or login, the API returns an authentication
token.
...
Orders are created from an existing offer detail. The order stores a snapshot
of the selected offer detail so later offer changes do not change existing
orders.
...
The default page size is configured by the backend, while clients can request a
different size with the `page_size` query parameter.
...
During local development they are served through Django when debug mode is
enabled.
...
If the frontend runs on a different host or port, update
...
The project uses Django's administration interface for managing application
data.
...
The admin interface provides access to users, profiles, authentication tokens,
...
The backend should be maintained in its own repository, separate from the
frontend.
...
Migration files are part of the source code and should remain in version
control.
