...
class AuthApiTests(APITestCase):
    def setUp(self):
        self.customer = self._create_user("customer_test", "customer")
        self.business = self._create_user("business_test", "business")
    ...
    def _profile_url(self, user):
        return reverse("profile-detail", kwargs={"pk": user.id})
    ...
    def test_login_returns_token(self):
        response = self.client.post(
            reverse("login"),
            {"username": "customer_test", "password": "TestPassword123!"},
        )
    ...
    def test_login_rejects_wrong_password(self):
        response = self.client.post(
            reverse("login"),
            {"username": "customer_test", "password": "wrong"},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_profile_requires_authentication(self):
        response = self.client.get(self._profile_url(self.customer))
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_profile_owner_can_update_profile(self):
        self._authenticate(self.customer)
        response = self.client.patch(
            self._profile_url(self.customer),
            {"first_name": "Ahmet", "location": "Siegen"},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["first_name"], "Ahmet")
        self.assertEqual(response.data["location"], "Siegen")
